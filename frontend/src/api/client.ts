...
export function originalImageUrl(jobId: string): string {
  return apiUrl(`/api/jobs/${jobId}/original`);
}
...
export interface LlmEditRegion {
  x: number;
  y: number;
  width: number;
  height: number;
...
export interface LlmEditRequest {
  ...
  /** User-space rectangle the user lassoed in marquee mode. The server
   * uses it to scope Grok's edits to a specific region of the SVG. */
  region?: LlmEditRegion;
  model?: string;
}
...
export interface LlmEditError extends Error {
  status: number;
  code?: string;
  /** Quota header value when the server returned one (e.g. "0", "5", "unlimited"). */
  quota?: string | null;
}

export async function llmEditSvg(req: LlmEditRequest): Promise<LlmEditResponse> {
  let res: Response;
  try {
    res = await fetch(apiUrl("/api/editor/llm-edit"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req),
    });
  } catch (err) {
    throw wrapFetchError(err, "llm edit");
  }

  const quota = res.headers.get("X-Editor-Quota-Remaining");
  if (!res.ok) {
    let detail = `Edit failed (${res.status})`;
    ...
    const err: LlmEditError = Object.assign(new Error(detail), {
      status: res.status,
      code,
      quota,
    });
